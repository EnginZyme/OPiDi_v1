...
class TestSimulationBlueprint(BaseTestCase):
    def test_protocol_file_string_generation(self):
        """Tests the generation of the simulation output for a given protocol object.
        """
        with self.client:
            response = self.client.post("/api/1/generator/simulate/", data=json.dumps(sample_protocol_payload),
